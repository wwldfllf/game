...
export class GameScene extends Phaser.Scene {
  ...
  makeWorld() {
    ...
    road.fillStyle(0x0a1718, 0.95); road.fillRect(0, WORLD / 2 - 280, WORLD, 560); road.fillRect(WORLD / 2 - 280, 0, 560, WORLD);
    road.lineStyle(2, 0x3c5750, 0.55);
    for (let p = 80; p < WORLD; p += 100) { road.lineBetween(p, WORLD / 2 - 6, p + 58, WORLD / 2 - 6); road.lineBetween(p, WORLD / 2 + 6, p + 58, WORLD / 2 + 6); road.lineBetween(WORLD / 2 - 6, p, WORLD / 2 - 6, p + 58); road.lineBetween(WORLD / 2 + 6, p, WORLD / 2 + 6, p + 58); }
    for (let i = 0; i < 58; i += 1) {
      const x = (i * 617) % (WORLD - 140) + 70; const y = (i * 383) % (WORLD - 140) + 70;
      const prop = this.add.image(x, y, i % 5 === 0 ? 'debris' : 'debris').setDepth(-12).setAlpha(0.5 + (i % 4) * 0.1).setScale(0.65 + (i % 3) * 0.18).setAngle((i * 37) % 360);
      if (Math.abs(x - WORLD / 2) < 350 || Math.abs(y - WORLD / 2) < 350) prop.setAlpha(0.24);
    }
  ...
  startRun(difficulty = 'normal') {
    ...
    this.player.body.setCircle(19, 29, 35); this.player.setCollideWorldBounds(true);
  ...
  updatePlayer(dt) {
    ...
    if (move.lengthSq() > 0.01) this.player.angle = Phaser.Math.RadToDeg(Math.atan2(move.y, move.x)) + 90;
    ...
    this.player.setAlpha(this.player.getData('invuln') > 0 ? 0.56 + Math.sin(this.elapsed * 28) * 0.2 : 1);
  ...
  updateEnemies(dt) {
    this.enemies.children.iterate((enemy) => {
      ...
      enemy.rotation += (enemy.enemyType === 'runner' ? 0.018 : 0.008) * dt * 60;
    });
  }
  ...
  endRun(victory) { if (this.mode === 'result') return; this.mode = 'result'; this.physics.world.pause(); const result = { victory, elapsed: this.elapsed, kills: this.kills, level: this.stats.level, combo: this.comboBest, score: this.score }; window.showResult?.(result); this.saveBest(result); if (victory) { this.burst(this.player.x, this.player.y, 0x8fffe5, 230); blip('level'); } }
